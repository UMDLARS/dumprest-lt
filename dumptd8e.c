/* This program receives an td8e Dectape image from the serial port from the
   PDP8 dump program.  It will prompt for the file to receive or use first
   command line argument. It needs a config file dumprest.cfg or
   $HOME/.dumprest.cfg with the format defined in config.c
      ...
      for (i = 0; i < c; i++) {
         if (count < 0 ) {
            ...
            if (count == -2) {
              ...
              /* Count >= 0 is word count in block */
              /* Byte is byte in 3 byte sequence for 2 words */
           if (byte == 0) {
              temp = buf[i];
              byte++;
           } else
           ...
           if (byte == 2) {
              ...
              /* If at end of block setup for next */
           if (count == 129) {
              count = -1;
              block++;
              byte = 0;
